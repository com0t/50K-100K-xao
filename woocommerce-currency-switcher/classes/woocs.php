...
//*** checking and assigning data
        ...
        if (isset($_REQUEST['woocs_cron_running']) AND!$email_is_sent) {
            if (get_option('woocs_rate_auto_update_email', 0)) {
                $message = sprintf('<strong>' . esc_html__('Base currency of the site is: %s', 'woocommerce-currency-switcher') . '</strong>', $this->default_currency);
                $message .= '<br /><br /><ul>';
    ...
    public function admin_enqueue_scripts() {
        if (isset($_GET['tab']) AND $_GET['tab'] == 'woocs') {
            ...
            if ($this->get_admin_theme_id() === 1) {
                wp_enqueue_style('woocs-google-fonts', 'https://fonts.googleapis.com/css?family=Nunito%3A300%2C400%2C600%2C700&subset=latin%2Clatin-ext', [], WOOCS_VERSION);
...
//+++
        ...
        if ($this->get_admin_theme_id() === 1) {
            wp_enqueue_style('woocs-unicons', 'https://unicons.iconscout.com/release/v3.0.6/css/line.css', [], WOOCS_VERSION);
