...
class WOOCS_STARTER {
    ...
    public function get_actual_obj() {
        ...
        if (version_compare($this->actualized, '3.3', '<')) {
            ...
            //from 05-05-2021 woocommerce versions lower v.3.3 not supported
        } else {
...
add_filter('option_woocommerce_price_thousand_sep', function($value) {
    ...
    if (is_object($WOOCS)) {
        ...
        if (isset($currencies[$current_currency]['separators'])) {
            switch (intval($currencies[$current_currency]['separators'])) {
                case 1:
                    $value = '.';
                    break;
                case 2:
        ...
        $value= apply_filters('woocs_price_thousand_sep', $value, $current_currency);
    }
...
add_filter('option_woocommerce_price_decimal_sep', function($value) {
    ...
    if (is_object($WOOCS)) {
        ...
        $value= apply_filters('woocs_price_decimal_sep', $value, $current_currency);
    }
